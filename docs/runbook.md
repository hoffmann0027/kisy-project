...
## HA данных, политика Redis и диск (O7)

### Postgres — выбран управляемый путь

Прод-БД живёт на **Neon** (см. `render.yaml`: `DATABASE_URL` задаётся в
дашборде). Это осознанный выбор «прагматичного пути»: HA, failover и PITR
обеспечивает провайдер, без primary/standby своими руками. Managed-бэкапы
**дополняют, а не заменяют** наш собственный дамп из O2 — провайдера можно
потерять целиком (аккаунт, оплата, регион), поэтому offsite-копия остаётся.

**Если БД недоступна.** Бэкенд не стартует и падает с
`postgres: init migrator` / `connected to postgres` в логах, `/ready` отдаёт
503, срабатывает `ReadinessFailing`. Порядок:

1. Проверить статус провайдера (Neon status / дашборд проекта).
2. Если это временная недоступность — дождаться, сервис поднимется сам
   (`restart: unless-stopped`, на Render — автоперезапуск).
3. Если БД потеряна — восстановление из бэкапа в новую базу по разделу
   «Backup / restore», затем сменить `DATABASE_URL` и передеплоить.

**Self-hosted альтернатива** (если managed недоступен): потоковая репликация
primary → standby (`primary_conninfo`, слот репликации), промоушен standby
через `pg_ctl promote`, после чего приложение переключается сменой
`DATABASE_URL`. Это заметно тяжелее в эксплуатации и здесь сознательно не
разворачивается — при одном администраторе managed надёжнее.

### Redis — состояние эфемерно, persistence выключен осознанно

В Redis лежит только восстановимое: presence-счётчики, каналы фан-аута,
состояние звонков в процессе и счётчики rate-limit. Источник правды — Postgres.
Поэтому в `docker-compose.yml` заданы явно:

| Параметр | Значение | Почему |
|---|---|---|
| `--save ""`, `--appendonly no` | persistence выключена | после рестарта лучше пустой Redis, чем снимок «кто был онлайн час назад» |
| `--maxmemory` | `REDIS_MAXMEMORY` (192mb) | без него Redis не знает про лимит контейнера (256M в prod) и получает OOM-kill вместо вытеснения |
| `--maxmemory-policy` | `allkeys-lru` | все ключи восстановимы; `noeviction` начал бы отклонять записи и сломал бы presence и звонки |

**Проверено на практике:** полное уничтожение Redis (`FLUSHALL` + рестарт
контейнера) не ломает приложение — сессии живы (они в Postgres + JWT), чтение,
новый логин и отправка сообщений работают (`201`), ключи отстраиваются сами.
Единственный видимый эффект: сбрасывается presence и рвутся активные звонки —
клиенты переподключаются.

Если когда-нибудь понадобится durable-состояние в Redis — включать
`--appendonly yes` (том `redis-data` уже смонтирован) либо переносить эти
данные в Postgres.

Управляемый Redis в проде — **Upstash**; его тариф сам управляет памятью,
поэтому наши флаги касаются только self-hosted стека.

### Диск

- **Логи контейнеров ограничены** (`x-logging` в `docker-compose.yml`):
  10 МБ × 3 файла на сервис. Раньше json-file рос без предела — а полный диск
  кладёт Postgres и вместе с ним весь сайт.
- **Файлы больше не в БД** после O5: рост переехал в объектное хранилище, у
  которого свои lifecycle-политики (настраиваются у провайдера).
- **Алерт `LowDisk`** из O1 (<15% свободного) обязателен — это последняя линия
  обороны; по нему смотреть логи, бакет и размер БД.
