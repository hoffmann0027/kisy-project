...
> **Обновлено (этап 2, миграция 000042).** Два вида аккаунтов, и разница
> между ними задаётся один раз — в момент регистрации.
>
> **Пригласительный** (`account_kind = 'invited'`) — всё, что описано ниже:
> пришёл по токену CEO, несёт уровень 1–10, участвует в иерархии.
>
> **Обычный** (`account_kind = 'basic'`) — зарегистрировался сам, **уровня не
> несёт вовсе** (`role_id IS NULL`). Это не «десятый уровень»: он не внизу
> иерархии, он вне её. Поэтому всё, что на иерархии построено — рейтинг,
> условия повышения, голосования, администрирование, пороги видимости групп —
> к нему не применяется и ему не показывается. Чаты, группы, сообщества,
> звонки, заметки и E2EE — работают одинаково у обоих.
>
> Ограничен обычный аккаунт не тем, кому он пишет, а тем, кого может найти:
> директории сотрудников у него нет, только поиск по полному имени. Открытую
> регистрацию можно выключить на конкретном деплое (`REGISTRATION_OPEN=false`),
> и тогда всё вернётся к строке ниже.

> **Обновлено (миграция 000046): логин и имя.** Логин (`username`) уникален
> без учёта регистра с самого начала (`CITEXT UNIQUE`). Отображаемое имя
> (`display_name`) — теперь тоже уникально, без учёта регистра и повторных
> пробелов, и подчиняется правилу: **только буквы латиницы и кириллицы,
> одиночные пробелы между словами, от 2 до 40 символов**. Имя спрашивается при
> регистрации. Правило одно: `internal/users/displayname.go` (регистрация,
> профиль, bootstrap CEO), `kisy_display_name_valid` в БД как страховка,
> `shared/lib/displayName.ts` на клиенте — с ошибками «Имя занято» /
> «Только буквы…».
>
> Существующие имена **не переписываются**. Невалидные и повторяющиеся (кроме
> самого раннего владельца) помечаются `display_name_needs_change`; такой
> пользователь при входе попадает на блокирующий экран выбора имени. Пока
> помеченные есть, уникальность и правило действуют для всех остальных; когда
> их не останется — `scripts/finalize-display-names.sh` делает их
> безусловными. Поиск обычного аккаунта по полному имени сравнивает тот же
> нормализованный ключ (`kisy_display_name_key`).
