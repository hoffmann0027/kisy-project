...
реакций.** Таблица `reactions` устроена как `UNIQUE (target, user_id,
emoji)`, то есть один человек может поставить десяток разных эмодзи. Если
считать строки, пост накручивается в одиночку. Одно мнение — один голос.
