...
Отметка «подтверждено» у аккаунтов, групп и сообществ (миграция 000047:
`users.verified_at`, `groups.verified_at`, плюс `verified_by`).

- **Выдаёт и снимает только CEO** — вкладка «Верификация» в админке: поиск
  по логину, имени или названию, кнопка «Выдать»/«Снять». Бэкенд:
  `GET /admin/verification`, `PUT /admin/users/{id}/verification`,
  `PUT /admin/groups/{id}/verification`, все под `RequireInvited` +
  `RequireClearance(CEO)`. Проверки подключаются вместе с маршрутами
  (`admin.Mount`), и тест обходит все маршруты `/admin`, проверяя 403 для
  не-CEO.
- **Аудит:** `user.verified` / `user.unverified` / `group.verified` /
  `group.unverified` — кто и когда. Повторная выдача не сдвигает дату.
- **Показ:** значок рядом с именем — профиль, строка чата, шапка чата, список
  участников и заявок, результаты поиска людей, карточка поста в ленте,
  заголовок сообщества. Клиенты узнают об изменении через `user.updated` /
  `group.changed`.
- **Значок** — `shared/ui/VerifiedBadge.tsx`: восьмигранная печать с галочкой
  (грани, а не зубцы — не повторяет розетки X/Instagram/Telegram). Два
  размера: 20px с внутренним ободком, 14px без него. Цвета — только токены
  темы `--acc` / `--acc-on`, поэтому значок работает во всех семи темах.
  Варианты, из которых выбирали, — `design/verified-badge-preview.html`.

## Модерация сообществ и групп (CEO)

Миграция 000048: `groups.deleted_at/deleted_by`, таблица `group_sanctions(id,
group_id, kind 'warn'|'mute'|'delete', reason NOT NULL, issued_by, issued_at,
expires_at, revoked_at, revoked_by, revoke_note)`. Код — `internal/moderation`.

| Санкция | Что делает |
|---|---|
| **Предупреждение** | Считается, пока не снято. **Третье активное удаляет** сообщество — так же, как удаление вручную: та же запись `delete`, то же уведомление. Снятое не считается. |
| **Мут** | 1 день / 7 дней / 30 дней / бессрочно. Пока активен, посты сообщества **не попадают в `GET /feed`** — фильтр в SQL (`internal/posts`: и страница, и входные данные рейтинга), не на клиенте. Внутри сообщества публиковать можно. Новый мут заменяет прежний. Рейтинг популярности в Redis пересчитывается сразу при муте, снятии, удалении и восстановлении; истечение срока подхватывает SQL мгновенно, рейтинг — на ближайшем 5-минутном пересчёте. |
| **Удаление** | Мягкое: сообщество исчезает для всех, но **30 дней** его можно восстановить со всеми постами, участниками и сообщениями. Затем фоновая задача раз в сутки удаляет его окончательно (`group.purged` в аудите). Удаление основателем через меню группы — по-прежнему сразу и окончательно; сообщество, удалённое модерацией, основатель окончательно удалить не может. |

**Восстановление** (вкладка «Удалённые»): активные предупреждения сокращаются
до двух — следующее снова удалит. Если их было меньше двух, остаются как есть:
восстановление не выдумывает предупреждений, которых никто не выносил.
Основатель получает уведомление.

**Причина обязательна** (1–1000 символов) и показывается получателям.
**Уведомления** (`internal/notifications`, тип `group_sanction`, + push) — основателю
и всем участникам с ролью «владелец», «редактор», «модератор»: «Сообществу X
вынесено предупреждение (2 из 3): причина», «замучено до …», «удалено: причина»;
о восстановлении — основателю. **Баннер** с текущими санкциями — внутри
сообщества для тех же людей (`GET /groups/{id}/sanctions`, остальным 403).

**Доступ:** все эндпоинты санкций — под `/admin` (`RequireInvited` +
`RequireClearance(CEO)`, подключаются `admin.Mount`); тест обходит все маршруты
`/admin` и требует 403 для не-CEO. Сервис дополнительно проверяет CEO сам.
**Аудит:** `group.warned`, `group.muted`, `group.moderation_deleted`,
`group.sanction_revoked`, `group.restored`, `group.purged`.
